...
public class BlockFarmLandBaseComponentImpl extends BlockBaseComponentImpl {
    ...
    public void onRandomUpdate(BlockStateWithPos current) {
        ...
        if (!hydrated(current)) {
            if (moisture > 0) {
                dimension.setBlockState(pos, current.blockState().setProperty(BlockPropertyTypes.MOISTURIZED_AMOUNT, moisture - 1));
            } else {
            ...
            dimension.setBlockState(pos, current.blockState().setProperty(BlockPropertyTypes.MOISTURIZED_AMOUNT, 7));
        }
    }
